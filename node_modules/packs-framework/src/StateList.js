...
export default class StateList {
    constructor(initial, recorder) {
        this.initial = initial;
        this.record = recorder == null ? recordFire : recorder;
        this.state = [
            {
                data: [],
                screenState: []
        ...
        if (!sessionStorage.getItem("key")) {
            (async () => {
                try {
                    ...
                    await this.record(endpoint, this.initial, "initial");
                    this.publish();
                } catch (e) {
    ...
    set(screenState) {
        this.state[this.index] = {
            ...this.state[this.index],
            screenState: this.state[this.index].screenState.concat({
                ...screenState,
                timestamp: Date.now()
            })
        };
    ...
    push(data, component,module_name, silent = false) {
        this.state[this.index].data = this.state[this.index].data.concat({
            ...data,
            [`module_${this.index}_component`]: component,
            [`module_${this.index}_t`]: Date.now()
        });
        if(module_name!=''){ this.state[this.index].module_name = module_name;}
       
        this.state[this.index].silent = silent;
        (async () => {
            try {
                const endpoint = echo(this.initial.endpoint, this.initial);
                await this.record(endpoint, this.state[this.index], this.index);
            } catch (e) {
                console.error(e, e.stack);
                this.initial = {
                    ...this.initial,
        ...
        if (!silent) {
            history.pushState(this.index, null, `?index=${this.index}`);
        }
        history.replaceState(this.index, null, `?index=${this.index}`);
        this.publish();
    }
    ...
    getState() {
        ...
        const datas = this.state.slice(0, this.index).map(s => fold(s.data));
        return {
            index: this.index,
